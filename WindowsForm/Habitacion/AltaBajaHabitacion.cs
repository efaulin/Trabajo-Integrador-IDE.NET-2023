...
namespace WindowsForm
{
    public partial class AltaBajaHabitacion : Form
    {
        ...
        public AltaBajaHabitacion(int opcion)
        {
            op = opcion;
            InitializeComponent();
        }
        ...
        private void guardar(object sender, EventArgs e, Habitacion tmpHbt)
        {
            try
            {
                Negocio.Habitacion.Update(tmpHbt);
                MessageBox.Show("Habitacion ID: " + tmpHbt.IdHabitacion + " editada con exito.");
                cmbIdHabitacion_SelectionChangeCommitted(sender, e);
            }
            catch
            {
                MessageBox.Show("Hubo un problema. Intente nuevamente", "Error", MessageBoxButtons.OK, MessageBoxIcon.Information);
                //throw ex;
            }
        }

        private void AltaBajaHabitacion_Load(object sender, EventArgs e)
        {
            if (_lstHbt.Count <= 0)
            {
